import { _languages, Dictionary, fetchTranslator, translateDict } from "@/libs/i18n";
import { Language } from "@/types/d";
import { Accessor, createContext, createResource, createSignal, InitializedResource, JSX, Setter, useContext } from "solid-js";
...
export function I18nProvider(props: {lang: Language, children: JSX.Element}){
    ...
    const [t] = createResource(lang, fetchTranslator, {initialValue: translateDict(DeDict)})
    const languages = () => _languages
    const languageProvider = {lang, setLang, t, languages}
     

    return (
        <LanguageContext.Provider value={languageProvider}>{props.children}</LanguageContext.Provider>
    )
}
